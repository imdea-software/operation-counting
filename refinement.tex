...
% XXX MJE DOES NOT UNDERSTAND THE VALUE OF THIS PARAGRAPH
% Libraries are abstractly defined by their interface with the clients, i.e., by
% considering the set of all possible sequences of method calls and returns,
% with their associated input and output values, that can be observed when the
% library methods are invoked by any program.
...
\begin{definition}
  ...
  A \emph{program} $P$ over actions $@S$ such that $@S \cap (C \cup R) = @0$ is
  an LTS over alphabet $(@S \cup C \cup R)$ such that each execution $e \in
  E(P)$ is well formed, and
  \begin{itemize}
...
possible in its composition with $L_1$ is also possible with $L_2$.
\begin{definition}

...
The executions in the premise and resp., the conclusion of each of the closure rules given
above %(together with the prefix closure which holds because all the states of an LTS are accepting) 
are related by a partial order $\sqsubseteq$ defined as follows: $e_1\sqsubseteq e_2$ iff
there exists an execution $e_1'$ obtained from $e_1$ by
...
\end{itemize}
We defer the reader to Section~\ref{} for the relation between $\sqsubseteq$ and the classic linearizability 
relation between executions~\cite{journals/toplas/HerlihyW90}.
...
It is straightforward that $e_1\sqsubseteq e_2$ whenever 
$e_1$ and $e_2$ are in the conclusion and respectively the premise of a closure rule. 
Therefore,

\begin{lemma}\label{lemma:exec_closure}

The set of executions $E(L)$ of a library $L$ is downward closed w.r.t. $\sqsubseteq$, i.e., 
if $e\in E(L)$ then $e'\in E(L)$ for every $e'\sqsubseteq e$.
...
Lemma~\ref{lemma:exec_closure} implies that $E(L)$ has a canonical representation,
which is the set of maximal elements w.r.t. $\sqsubseteq$, called the \emph{kernel} of $L$ 
and denoted by $\kernel(L)$. 

A library $L$ is called \emph{atomic} iff its kernel contains only sequential executions, i.e., executions where 
every call action is immediately followed by its matching return action. 
Atomic libraries are typically used as specifications for concurrent objects.

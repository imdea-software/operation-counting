...
\begin{proof}
  ...
  element of $e_2' \in E_2'$ must be $~>$-derivable from some $e_1' \in
  E_1'$, and visa versa, since $e_2' \in \overline{E}_1$ and $e_2'$ is
...
% The executions in the premise and resp., the conclusion of each of the closure
% rules given above
% % (together with the prefix closure which holds because all the states of an LTS
% % are accepting)
% are related by a partial order $\sqsubseteq$ defined as follows:
% $e_1\sqsubseteq e_2$ iff there exists an execution $e_1'$ obtained from $e_1$
% by appending matching return actions (for unmatched call actions) or deleting
% call actions such that
%
% \begin{itemize}
%
%   \item $e_1'$ and $e_2$ contain exactly the same set of actions, and
%
%   \item $e_2$ is a permutation of $e_1'$ that preserves the order between
% return and call actions (i.e., if a return action $r\in R$ occurs before a call
% action $c\in C$ in $e_1'$ then the same holds in $e_2$).
%
% \end{itemize}
% We defer the reader to Section~\ref{} for the relation between $\sqsubseteq$
% and the classic linearizability relation between
% executions~\cite{journals/toplas/HerlihyW90}.
%
% \begin{example}
% TODO ILLUSTRATE THE RELATION $\sqsubseteq$. POSITIVE AND NEGATIVE EXAMPLES.
% \end{example}
%
% It is straightforward that $e_1\sqsubseteq e_2$ whenever $e_1$ and $e_2$ are in
% the conclusion and respectively the premise of a closure rule. Therefore,
%
% \begin{lemma}
%   \label{lemma:exec_closure}
%
%   The set of executions $E(L)$ of a library $L$ is downward closed w.r.t.
%   $\sqsubseteq$, i.e., if $e\in E(L)$ then $e'\in E(L)$ for every
%   $e'\sqsubseteq e$.
%
% \end{lemma}
%
...
% \end{proof}
%
% Lemma~\ref{lemma:exec_closure} implies that $E(L)$ has a canonical
% representation, which is the set of maximal elements w.r.t. $\sqsubseteq$,
% called the \emph{kernel} of $L$ and denoted by $\kernel(L)$.
%
% A library $L$ is called \emph{atomic} iff its kernel contains only sequential
% executions, i.e., executions where every call action is immediately followed by
% its matching return action. Atomic libraries are typically used as

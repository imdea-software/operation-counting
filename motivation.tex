...
\node[lnode] (x5)  [left= .7cm of y7,label=left:$^{8}$,yshift=-2mm] {}; %label=above:$x$
\node[lnode] (x6)  [below of=x5,label=left:$\<ret>(3)$] {}; 
\node[lnode] (x7)  [below of= x6,label=left:${\tt x=3}$] {}; 
\node[lnode] (x8)  [below of= x7,label=left:$\<pop>()$] {}; 
\node[lnode] (x9)  [below of =x8,label=left:$\<ret>({\tt EMPTY})$] {}; 
\node[lnode] (x10)  [below of= x9,label=left:${\tt y=EMPTY}$] {}; 

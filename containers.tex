...
was detected with approximation $A_1$. \alert{Furthermore, the history in Figure~\ref{fig:stacks}(b)
satisfies the formula $@Y_\mathrm{ev}$ and there exists a weaker Treiber's stack history 
of length 1 (the history in Figure~\ref{fig:stacks}(c) without one of the pending $\<pop>$ operations),
which also satisfies $@Y_\mathrm{ev}$.}

...
Note that for every family $@Y$ defined in Figure~\ref{fig:spec:ds}, the size of the $i$th formula
$@Y_i$ is linear in $i$, and its quantifier rank is at most 4.
...
approximation is guaranteed to discover any stack/queue violation in
...
\alert{Similarly, the history in Figure~\ref{fig:stacks}(b)
satisfies $@Y_\mathrm{ev}$ and there exists a weaker Treiber's stack history 
of length 1 (the history in Figure~\ref{fig:stacks}(c) without one of the pending $\<pop>$ operations),
which also satisfies $@Y_\mathrm{ev}$. On the other hand, in practice, we have found that if a library
has an empty violation, then it also has an ordering violation. For example, if we consider the execution
in Figure~\ref{fig:stacks}(a), if the first thread would have executed another $\<push>(4)$ operation before
$\<push>(1)$, then the last $\<pop>$ operation in the execution would have returned $4$ and the execution
would reveal an ordering violation instead of an empty violation.}

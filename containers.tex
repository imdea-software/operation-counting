...
These cutoffs allow us to formulate a partial completeness of our approximation
...
\begin{lemma}
  ...
  If $h$ is a history without completed $\<pop>(\mathtt{EMPTY})$ operations and
  $x \in \set{ \mathrm{set}, \mathrm{stack}, \mathrm{queue}}$, then $h \in
  H(L_x)$ if{f} $h |= @Y_x$.

\end{lemma}

Thus our $A_3$ approximation is guaranteed to discover any stack violation in
histories using unique values, modulo empty violations. Unfortunately the
empty-violation formula $@Y_\mathrm{ev}$ is not complete in general, as the

...
\begin{proof}
  ...
  \begin{align*}
    & \text{for each } q \in Q, o \in O, m \in \<Methods> \\
    & \text{such that } f(o) = \tup{m,\_},
      \lnot q(o), \text{ and } q(o') \text{ for all } o' < o \text{:} \\
    & \qquad q \xrightarrow{\call{m}{o}} q \\
    & \text{for each } q \in Q, o \in O, m \in \<Methods>
      \text{ such that } f(o) = \tup{m,\<true>} \text{:} \\
    & \qquad q[o |-> \<false>] \xrightarrow{\ret{o}} \cdot \xrightarrow{a} q[o |-> \<true>]
  \end{align*}
  In any execution $e \in E(P_h)$, the call action $\call{m}{o}$ of any

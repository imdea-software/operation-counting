...
\begin{proof}
  ...
  \begin{align*}
    & \text{for each } q \in Q, o \in O, m \in \<Methods>, u \in \<Vals> \\
    & \text{such that } f(o) = \tup{m,u,\_},
      \lnot q(o), \text{ and } q(o') \text{ for all } o' < o \text{:} \\
    & \qquad q \xrightarrow{m(u)_o} q \\
    & \text{for each } q \in Q, o \in O, m \in \<Methods>, v \in \<Vals>
      \text{ with } f(o) = \tup{m,\_,v} \text{:} \\
    & \qquad q[o |-> \<false>] \xrightarrow{\<ret>(v)_o} \cdot \xrightarrow{a} q[o |-> \<true>]
  \end{align*}

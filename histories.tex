...
\alert{Any given history $h$ of a library $L_1$ can be captured by a program 
whose observations witness the ordering among operations; 
observational refinement $L_1\leq L_2$ then implies that $L_2$ must also allow 
those observations, and thus the history $h$. For the reverse, by Lemma~\ref{lem:lib:execs},
a library must admit all the executions with a given history and consequently, $H(L_1) \subseteq H(L_2)$
implies that any program execution possible when using $L_1$ is also possible when
using $L_2$.}

...
\begin{example}
  ...
  pictured in Figure~\ref{fig:stacks}(b) (arrows depict the order relation 
   and some transitive orderings are omitted). Any two operations are ordered in $h$ 
  according to the order between their return, and respectively, call actions in the execution $e$.
  For example, the $\<push>(1)$ operation is ordered before the $\<pop>=>3$ operation in $h$ 
  because the return action of $\<push>(1)$ comes before the call action of $\<pop>=>3$ in $e$.
  On the other hand, the $\<pop>=>1$ operation is incomparable to the $\<pop>=>3$ operation
  because the return action of $\<pop>=>1$ comes after the call action of $\<pop>=>3$
  and the return action of $\<pop>=>3$ comes after the call action of $\<pop>=>1$.
  The order between the call (resp., return) actions of two operations is irrelevant for defining
  the history $h$.

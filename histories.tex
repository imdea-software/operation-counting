...
time: \alert{the actions in an execution are linearly ordered}.
...
\begin{itemize}
  ...
  \item $f_1(g(o)) \alert{\ll} f_2(o)$ for each $o \in O_2$.

\end{itemize}
where $(m_1(u_1) => v_1) \alert{\ll} (m_2(u_2) => v_2)$ if{f} $m_1 = m_2$, $u_1 =
...
between their \alert{sets of }histories. Before that, we give two lemmas which characterize the
...
equivalence between refinement and history inclusion. \alert{Thus, if 
$L_1 \leq L_2$, then for any history $h\in H(L_1)$, one can build a program $P_h$ 
which composed with the library $L_2$ has an execution $e$ such that $h\preceq H(e)$.
Since libraries are closed under weakening and $H(e)\in H(L_2)$, it follows that $h\in H(L_2)$.
For the reverse, if $H(L_1) \subseteq H(L_2)$, then every execution $e$ of some program $P$ 
composed with $L_1$ is reproducible when $P$ is composed with $L_2$. This follows from
the fact that $H(e)\in H(L_2)$ and from the closure property in Lemma~\ref{lem:lib:execs}.}

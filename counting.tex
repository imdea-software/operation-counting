...
\begin{algorithm}
  ...
  \Switch{input action}{
    ...
    \Case{return action $ \<ret>(v)_o$}{
      $i <-|\<time>-\nbbarr{e,m(u)_o}|_k$ \;
      $j <- \<min>(\Time,k)$ \;
      \Decrement $\Counters[\synop{m}{u}{\bottom},i,k]$ \;
      \Increment $\Counters[\synop{m}{u}{v},i,j]$ \;
      $\NewPast <- \True$
    }
  }
  \Yield $\Counters$ 
  ...
  \Indp
  \ForEach{$x=\synop{m}{u}{v}$}{
  	\For{$i$ = $0$ \To $k-1$}{
		\For{$j$ = $i$ \To $k-1$}{
			$\Counters[x,i,j]$ += $\Counters[x,i+1,j+1]$
			$\Counters[x,i+1,j+1] <- 0$ 
		}
	}
	\For{$j$ = $0$ \To $k-1$}{
			$\Counters[x,0,j]$ += $\Counters[x,0,j+1]$
			$\Counters[x,0,j+1] <- 0$ 
	}
  }
  \ForEach{$x=\synop{m}{u}{\bottom}$}{
	\For{$i$ = $0$ \To $k-1$}{
			$\Counters[x,i,k]$ += $\Counters[x,i+1,k]$
			$\Counters[x,i+1,k] <- 0$ 
	}
  }

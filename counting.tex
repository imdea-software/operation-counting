...
\todo{I used ``closed-form expression'' but I begin to have doubts about this ... I think that it's more appropriate the term ``symbolic representation'' ... because our closed-form expressions are always boolean ... and I feel that this is not the right spirit}
...
in terms of \emph{pasts} %and \emph{futures} 
...
The following result states that every two pasts %or futures 
in an interval
order are ordered by set inclusion.

\begin{lemma}[Rabinovitch~\cite{Rabinovitch197850}]

  A partial order $\tup{O,<}$ is an interval order iff the sets $\past{o}$ with $o\in O$ %and respectively,  the sets $\future{o}$ with $o\in O$, 
...
representation of an execution $e$'s history. 
...
Thus, given a library execution $e$, we consider 

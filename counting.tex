...
\end{align*}
where $h = \tup{O,f,<}$. The \emph{quantifier rank} of an $\ocl$ formula $@Y$
is the maximum number of nested quantifiers in $@Y$.
...
\begin{proof}
  ...
  $@Y$ are only instantiated over labels occurring in $h$. The latter implies
  that quantifiers can be replaced by a disjunction over the labels occurring
  in $h = \tup{O,f,<}$:
  \begin{align*}
    h |= \exists x.\ F
    \quad \text{if{f}} \quad
...
Also, $A_k(h)=h$ for any history $h$ of length $n\leq k$.
The next lemma relates the interval maps of $h$ and $A_k(h)$.

\begin{lemma}\label{lemma:abstraction}

Let $I : O -> [n]^2$ and $I_k: O -> [k]^2$ be the interval maps
of $h$ and $A_k(h)$, respectively. Then, for every $o\in O$,
\[
I(o)=[i,j] \mbox{ iff } I_k(o) = [ |n-i|_k, |n-j|_k ]
\]
...
$A_k(H(e))$, for any given $k$ and $e$. The counting
...
\begin{algorithm}[thb]
  ...
  \Switch{input action}{
    \Case{call action $m(u)_o$}{
      ...
      $\StartTime[o] <- \Time$ \;
      \Increment $\Counters[m(u) => \bottom,\StartTime[o],k]$ \;
    }
    \Case{return action $ \<ret>(v)_o$}{
%      $i <-|\<time>-\nbbarr{e,m(u)_o}|_k$ \;
%      $j <- \<min>(\Time,k)$ \;
      \Decrement $\Counters[m(u) => \bottom,\StartTime[o],k]$ \;
  ...
  \ForEach{$@l = m(u) => v$}{
  	\For{$i$ = $1$ \To $k$}{
		\For{$j$ = $i$ \To $k$}{
			$\Counters[@l,i-1,j-1]$ += $\Counters[@l,i,j]$
			$\Counters[@l,i,j	] <- 0$ 
		}
	}
	\For{$j$ = $1$ \To $k$}{
			$\Counters[@l,0,j-1]$ += $\Counters[@l,0,j]$
			$\Counters[@l,0,j] <- 0$ 
	}
  }
  \ForEach{$@l = m(u) => \bottom$}{
	\For{$i$ = $1$ \To $k$}{
			$\Counters[@l,i-1,k]$ += $\Counters[@l,i,k]$
			$\Counters[@l,i,k] <- 0$ 
	}
  }
  \ForEach{$o$} {
    ...
    $\Pi(A_k(H(e)))$ of a given history $h$.}
  \label{alg:counting}
\end{algorithm}
...
\begin{lemma}
  ...
  $\Pi(A_k(H(e)))$ of the weakening $A_k(H(e))$ of a given execution's 
  history $H(e)$ in space $O(|\<Methods>| \cdot |\<Vals>|^2 \cdot \log |e|)$
...
generates the set $\set{ A_k(h) : h \in H(L) }$ of all recency abstractions of histories of $L$.
%More precisely, every value of the array $\<counters>$ reachable in this program 
%represents the counting representation $\Pi(A_k(h))$ of some $A_k(h)$ with $h\in H(L)$
...
\begin{lemma}
  ...
  For any library $L$,
  \[
  \<Cnts>(P_k\times L) = \set{ \Pi(A_k(h)) : h \in H(L) }.
  \] 
...
\begin{theorem}
  ...
  the approximate inclusion $\exists h.\ A_k(h) \in H(L_1) \setminus H(L_2)$ is equivalent to
  the fact that 

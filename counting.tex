...
\begin{example}
  ...
  Figure~\ref{fig:stacks}(d) pictures the canonical representation of the history $h$ in
...
\begin{example}
 ...
 \begin{align*}
 \{\!\{&\tup{\<push>(1),[0,0]}, \tup{\<pop>=>1,[0,0]},\tup{\<push>(2),[0,0]}, \\
 &\tup{\<push>(3),[0,0]},\tup{\<pop>=>{\tt Empty},[1,1]},\tup{\<pop>=>\bot,[1,1]},\\
 &\tup{\<pop>=>\bot,[1,1]}\}\!\}
 \end{align*}
\end{example}
...
\end{align*}
where $<'$ is the transitive closure of $\mathbin{<} \cup \set{ \tup{o',o} :
f(o') = \_(\_) => \bot}$ relating all completed operations to $o$. As the
...
\begin{theorem}
  ...
  $\Pi(A_k(H(e)))$ of an execution $e$ in $O(|\<Methods>| \cdot |\<Vals>|^2)$

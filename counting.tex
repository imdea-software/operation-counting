...
%      $j <- \<min>(\Time,k)$ \;
  ...
  \ForEach{$x=\synop{m}{u}{v}$}{
  	\For{$i$ = $1$ \To $k$}{
		\For{$j$ = $i$ \To $k$}{
			$\Counters[x,i-1,j-1]$ += $\Counters[x,i,j]$
			$\Counters[x,i,j	] <- 0$ 
		}
	}
	\For{$j$ = $1$ \To $k$}{
			$\Counters[x,0,j-1]$ += $\Counters[x,0,j]$
			$\Counters[x,0,j] <- 0$ 
	}
  }
  \ForEach{$x=\synop{m}{u}{\bottom}$}{
	\For{$i$ = $1$ \To $k$}{
			$\Counters[x,i-1,k]$ += $\Counters[x,i,k]$
			$\Counters[x,i,k] <- 0$ 
	}
  }
  \ForEach{$o$} {
...
\todo{Checking inclusion is reducible to invariant checking}

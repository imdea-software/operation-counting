...
to a weaker history $A_k(h)$ whose interval-length is bounded by $k$, and
...
logical characterization of sets of histories using arithmetic, inequalities,
and an operation-counting function $\#$. Formally, \emph{operation counting
logic} extends Presburger arithmetic with
\begin{itemize}
...
\begin{proof}
  ...
  has interval length at most $k$, it remains only to show $A_k(h) |= @Y$ is
  polynomial-time computable. This fact follows easily from \ldots
  CONSTANTIN? HELP??

\end{proof}

\subsection{Monitoring Histories by Counting}
...
\begin{algorithm}
  ...
  \Init{$\Time = 0$, $\NewPast = \False$, $\Counters[\_,\_,\_,\_] = 0$}
  \Switch{input action}{
    \Case{call action $\call{m}{o}$}{
      \If{$\NewPast$}{
        \Increment $\Time$ \;
        \If{$\Time > k$ and $A_k = A^{\triangleleft}_k$}{
          \Decrement $\Time$
        }
        \If{$\Time > k$ and $A_k = A^{\triangleright}_k$}{
          \Shift{\Counters}
        }
        $\NewPast <- \False$
      }
      $\StartTime[o] <- \Time$ \;
      \Increment $\Counters[m,\False,\StartTime[o],k]$ \;
    }
    \Case{return action $\ret{o}$}{
      \Decrement $\Counters[m,\False,\StartTime[o],k]$ \;
      \Increment $\Counters[m,\True,\StartTime[o],\Time]$ \;
      $\NewPast <- \True$
    }
  }
  \Yield $\Counters$
...
XXX FIGURE OUT HOW TO WORK IN THE FOLLOWING
